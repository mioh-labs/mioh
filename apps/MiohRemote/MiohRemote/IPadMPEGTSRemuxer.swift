...
enum IPadMPEGTSRemuxer {
  ...
  private static func writeModern(
    ...
    do {
      ...
      throw RemuxError.writer("writer開始: \(diagnostic(error))")
    }
  ...
  private static func writeLegacy(
    ...
    guard writer.canAdd(input) else {
      throw RemuxError.writer("video inputを追加できません")
    }
    writer.add(input)
    guard writer.startWriting() else {
      throw RemuxError.writer(
    ...
    let baseDTS = stream.units[0].dts
    let deltas = zip(stream.units.dropFirst(), stream.units).map {
      max(1, $0.dts - $1.dts)
    }
    let fallbackDuration = deltas.last ?? 3_003
    do {
...
enum IPadHLSIntervalAssembler {
  ...
  static func validateDecodableVideo(
    ...
    if #available(iOS 26.0, *) {
      try await validateModern(reader: reader, output: output)
    } else {
      try await validateLegacy(reader: reader, output: output)
    }
  }

  @available(iOS 26.0, *)
  private static func validateModern(
    ...
    do {
      ...
      throw AssemblyError.decode("reader開始: \(diagnostic(error))")
    }
    defer { reader.cancelReading() }
    do {
      while let sample = try await provider.next() {
        try Task.checkCancellation()
        if CMReadySampleBuffer<CVReadOnlyPixelBuffer>(sample) != nil {
          return
        }
      }
    } catch is CancellationError {
      ...
      throw AssemblyError.decode("frame取得: \(diagnostic(error))")
    }
