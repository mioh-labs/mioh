...
final class IPadInteractiveMediaBrowser: NSObject, ObservableObject {
  ...
  /// Pop-under scripts commonly finish before the user-requested navigation
  /// starts. Keep the opener visible briefly so its real navigation wins.
  private static let relaxedPopupAdoptionDelayNanoseconds: UInt64 = 500_000_000
  ...
  private var transientPopupAdoptionTask: Task<Void, Never>?
  ...
  private static func isHighConfidenceAdvertisementNavigationURL(_ url: URL?) -> Bool {
    ...
    let advertisementHostSuffixes = [
      ...
      "javhd-trk.com",
      "nettrck.store",
      // Pop-under networks observed on SupJav. These are navigation hosts,
      // not the embedded video or HLS origins.
      "qpon",
      "snapptrckr.fun",
  ...
  private func retireTransientPopup(_ expectedWebView: WKWebView? = nil) {
    ...
    transientPopupAdoptionTask?.cancel()
    transientPopupAdoptionTask = nil
  ...
  private func markTransientPopupReady(_ popupWebView: WKWebView) {
    ...
    if Self.relaxedWebCompatibilityEnabled {
      ...
      transientPopupAdoptionTask?.cancel()
      transientPopupAdoptionTask = Task { @MainActor [weak self, weak popupWebView] in
        do {
          try await Task.sleep(
            nanoseconds: Self.relaxedPopupAdoptionDelayNanoseconds
          )
        } catch {
          return
        }
        ...
        // A click can both navigate the opener and create a pop-under. If the
        // opener moved, keep that user-requested page visible and discard the
        // popup instead of replacing the screen with whichever response won
        // the network race.
        guard self.webView === openerWebView,
        ...
        guard !popupWebView.isLoading,
          Self.sanitizedPublicHTTPSURL(popupWebView.url)?.absoluteString
            == popupURL.absoluteString
        else { return }
        self.transientPopupAdoptionTask = nil
        self.showOpenedPage()
      }
      return
    }
    ...
    -> IPadBrowserMediaHandoffLease
  {
    ...
    while let activeState = mediaHandoffState {
      ...
      {
        // A same-document/player navigation can advance the browser generation
        // while the old lease is still unwinding. An explicit browser analysis
        // also supersedes any earlier preview owner. The lease UUID, not the
        // page generation, is the ownership token, so fully retire that exact
        // suspension before allowing the new route to acquire WebKit.
