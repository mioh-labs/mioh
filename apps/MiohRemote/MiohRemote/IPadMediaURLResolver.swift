...
struct IPadHLSSegmentEncryption: Sendable, Equatable {
  let keyResource: IPadHLSResource
  /// Explicit 128-bit IV from EXT-X-KEY. A missing value uses the media
  /// sequence number as required by RFC 8216.
  let initializationVector: [UInt8]?

  func resolvedInitializationVector(sequence: Int64) -> [UInt8] {
    if let initializationVector { return initializationVector }
    var result = [UInt8](repeating: 0, count: kCCBlockSizeAES128)
    var value = UInt64(sequence)
    for index in stride(
      from: result.count - 1,
      through: result.count - 8,
      by: -1
    ) {
      result[index] = UInt8(value & 0xff)
      value >>= 8
    }
    return result
  }
...
struct IPadHLSMediaSegment: Sendable, Equatable {
  ...
  let encryption: IPadHLSSegmentEncryption?
  ...
  init(
    ...
    encryption: IPadHLSSegmentEncryption? = nil
  ) {
...
struct IPadMediaURLResolver: Sendable {
  ...
  private static func parseMediaPlaylist(
    ...
    var segmentEncryption: IPadHLSSegmentEncryption?
    ...
    for line in lines {
      ...
      } else if uppercased.hasPrefix("#EXT-X-KEY:") {
        let attributes = parseAttributeList(
          String(line.dropFirst("#EXT-X-KEY:".count))
        )
        switch attributes["METHOD"]?.uppercased() {
        case "NONE":
          segmentEncryption = nil
        case "AES-128":
          let keyFormat = attributes["KEYFORMAT"]?.lowercased() ?? "identity"
          guard keyFormat == "identity",
            let uri = attributes["URI"],
            let keyURL = resolvedHTTPURL(uri, relativeTo: url)
          else {
            throw IPadMediaURLResolverError.invalidPlaylist(
              "AES-128鍵の指定が不正です"
            )
          }
          segmentEncryption = IPadHLSSegmentEncryption(
            keyResource: IPadHLSResource(url: keyURL, byteRange: nil),
            initializationVector: try attributes["IV"].map(parseAES128IV)
          )
        default:
          throw IPadMediaURLResolverError.encryptedPlaylist
        }
  ...
  private static func parseAES128IV(_ source: String) throws -> [UInt8] {
    let value = source.trimmingCharacters(in: .whitespacesAndNewlines)
    guard value.lowercased().hasPrefix("0x") else {
      throw IPadMediaURLResolverError.invalidPlaylist(
        "AES-128のIVに0x接頭辞がありません"
      )
    }
    var hex = String(value.dropFirst(2))
    guard !hex.isEmpty, hex.count <= 32,
      hex.allSatisfy({ $0.isHexDigit })
    else {
      throw IPadMediaURLResolverError.invalidPlaylist("AES-128のIVが不正です")
    }
    if hex.count % 2 != 0 { hex = "0" + hex }
    hex = String(repeating: "0", count: 32 - hex.count) + hex
    var bytes: [UInt8] = []
    bytes.reserveCapacity(kCCBlockSizeAES128)
    ...
    guard bytes.count == kCCBlockSizeAES128 else {
      throw IPadMediaURLResolverError.invalidPlaylist("AES-128のIV長が不正です")
    }
    return bytes
  }
...
final class IPadHLSResourceDownloader: @unchecked Sendable {
  ...
  private var encryptionKeyCache: [IPadHLSResource: Data] = [:]
  ...
  func materialize(
    ...
    let decryptedSegmentData: Data
    if let encryption = segment.encryption {
      let key = try await cachedEncryptionKey(
        for: encryption.keyResource,
        priority: priority
      )
      decryptedSegmentData = try Self.decryptAES128CBC(
        downloadedSegmentData,
        key: key,
        initializationVector: encryption.resolvedInitializationVector(
          sequence: segment.sequence
        )
      )
    } else {
      decryptedSegmentData = downloadedSegmentData
    }
    let segmentData = try Self.normalizedMediaSegmentData(
      decryptedSegmentData,
  ...
  private func cachedEncryptionKey(
    for resource: IPadHLSResource,
    priority: IPadSharedHTTPTransportOptions.Priority
  ) async throws -> Data {
    if let cached = cachedEncryptionKeyValue(for: resource) { return cached }
    let downloaded = try await data(for: resource, priority: priority)
    guard downloaded.count == kCCKeySizeAES128 else {
      throw IPadMediaURLResolverError.invalidPlaylist(
        "AES-128鍵が16 bytesではありません"
      )
    }
    try checkNotCancelled()
    storeEncryptionKeyValue(downloaded, for: resource)
    return downloaded
  }

  private func cachedEncryptionKeyValue(
    for resource: IPadHLSResource
  ) -> Data? {
    lock.lock()
    let value = encryptionKeyCache[resource]
    lock.unlock()
    return value
  }

  private func storeEncryptionKeyValue(
    _ data: Data,
    for resource: IPadHLSResource
  ) {
    lock.lock()
    if !isCancelled { encryptionKeyCache[resource] = data }
    lock.unlock()
  }

  private static func decryptAES128CBC(
    _ encrypted: Data,
    key: Data,
    ...
    guard key.count == kCCKeySizeAES128,
      ...
      !encrypted.isEmpty,
      encrypted.count % kCCBlockSizeAES128 == 0
    else {
      throw IPadMediaURLResolverError.invalidPlaylist(
        "AES-128暗号化区間のサイズが不正です"
      )
    }
    ...
    guard status == kCCSuccess else {
      throw IPadMediaURLResolverError.requestFailed(
        "AES-128区間を復号できませんでした（status \(status)）"
      )
    }
    decrypted.removeSubrange(outputCount..<decrypted.count)
    return decrypted
  }
