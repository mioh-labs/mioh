  ...
  static let page = #"""
    ...
    const settingTabs=[
      ...
      ['restoration',[
        ...
        ['ROIエンハンサー',[['roiEnhancer','方式','select','roiEnhancers'],['roiEnhancerModel','モデル','roiModel'],['roiEnhancerScale','倍率','number',1,8,1],['roiEnhancerStrength','強度','range',0,1,0.05],['roiEnhancerPasses','PiperSR反復','number',1,10,1],['roiEnhancerTile','タイル','number',0,1024,32]]]
      ]],
      ['detection',[
    ...
    function fieldDisabled(key){const s=config.settings;if(['useSegmentCount','segmentCount','segmentDuration','forceSplit'].includes(key)&&s.noSplit)return true;if(['roiEnhancerScale','roiEnhancerStrength','roiEnhancerTile'].includes(key)&&s.roiEnhancer==='none')return true;if(key==='roiEnhancerPasses'&&s.roiEnhancer!=='pipersr')return true;if(key==='quality'&&!s.useQuality)return true;if(key==='qmin'&&!s.useQMin)return true;if(key==='qmax'&&!s.useQMax)return true;if(['fps','fpsDenominator'].includes(key)&&!s.useFPS)return true;if(key==='preFPSConversion'&&!s.useFPS)return true;return false}
