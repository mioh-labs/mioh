...
final class MiniMaxH3Controller: ObservableObject {
  ...
  private static let retainGenerationCacheDefaultsKey =
    "com.okatti.mioh.upscaler.h3RetainGenerationCache"
  ...
  @Published var retainGenerationCache: Bool {
    didSet {
      UserDefaults.standard.set(
        retainGenerationCache,
        forKey: Self.retainGenerationCacheDefaultsKey
      )
    }
  }
  ...
  init() {
    ...
    retainGenerationCache = defaults.object(
      forKey: Self.retainGenerationCacheDefaultsKey
    ) as? Bool ?? true
    refreshConditioningMode()
  }
  ...
  private static func availableOutputURL(
    ...
    var stem = requested.deletingPathExtension().lastPathComponent
    var sequence = 2
    let expression = try? NSRegularExpression(
      pattern: #"^(.*) \(([0-9]+)\)$"#
    )
    let fullRange = NSRange(stem.startIndex..<stem.endIndex, in: stem)
    if let match = expression?.firstMatch(in: stem, range: fullRange),
      match.numberOfRanges == 3,
      let baseRange = Range(match.range(at: 1), in: stem),
      let numberRange = Range(match.range(at: 2), in: stem),
      let existingSequence = Int(stem[numberRange])
    {
      stem = String(stem[baseRange])
      sequence = max(2, existingSequence + 1)
    }
  ...
  func start() {
    ...
    let wasAutomaticOutput = outputPath == automaticOutputPath
    let availableOutput = Self.availableOutputURL(
      for: URL(fileURLWithPath: outputPath)
    )
    outputPath = availableOutput.path
    if wasAutomaticOutput {
      automaticOutputPath = availableOutput.path
    }
    ...
    task.terminationHandler = { [weak self] completed in
      Task { @MainActor in
        ...
        if completed.terminationStatus == 0 {
          self.progress = 1
          if self.retainGenerationCache {
            self.status = "MiniMax H3生成完了"
          } else {
            self.status = "生成完了・中間キャッシュを削除中"
            Task {
              do {
                ...
                self.status = "MiniMax H3生成完了・中間キャッシュ削除済み"
                self.appendLog("Generation cache removed: \(cache.path)\n")
              } catch {
                self.status = "生成完了・中間キャッシュ削除失敗"
                self.appendLog(
                  "Generation cache cleanup failed: \(error.localizedDescription)\n"
                )
              }
            }
          }
    ...
    do {
      ...
      appendLog("Core AI cache: system default\n")
      appendLog(
        "Generation cache after completion: "
          + (retainGenerationCache ? "keep\n" : "delete\n")
      )
  ...
  var body: some View {
    Form {
      Section("動画生成（MiniMax H3）") {
        ...
        Toggle(
          "生成完了後も中間キャッシュを残す",
          isOn: $controller.retainGenerationCache
        )
          .disabled(controller.isRunning)
        Text("オフにすると生成成功後にデコード・中間テンソル等の生成キャッシュを削除します。失敗・停止時は再開のため残します。Core AI特殊化キャッシュはシステム標準の場所を使用します。")
          .font(.caption)
          .foregroundStyle(.secondary)
