...
### Using macOS (with MPS / Apple Metal Support)

These steps describe installing the GUI/CLI on macOS and enabling MPS acceleration (Apple Silicon or Intel Macs with Metal):

- Install system dependencies (Homebrew):
  ```shell
  brew install python@3.12 gstreamer gst-plugins-base gst-plugins-good gst-plugins-bad gtk4 libadwaita pygobject3 ffmpeg
  ```
- Install PyTorch (MPS-enabled):
  ...
  PY
  ```
- Install Lada dependencies (GUI/CLI):
  ```shell
  python3.12 -m pip install -r packaging/requirements-gui.txt
  ```
- Start the GUI:
  ```shell
  python3.12 -m lada.gui.main
  ```
- Start the CLI with MPS:
  ```shell
  lada-cli --device mps --input <input video path>
  ```

> [!TIP]
> Prefer the startup script for macOS: it sets GI/GStreamer/GTK paths and points `LADA_MODEL_WEIGHTS_DIR`/`LOCALE_DIR` to the repo so you can launch from any directory.
> ```shell
> chmod +x scripts/run-lada-macos.sh
> ./scripts/run-lada-macos.sh gui   # or: ./scripts/run-lada-macos.sh cli [args]
> ```

> [!NOTE]
> On MPS, YOLO detection runs with `fp16` disabled for stability; when the device is `mps`, both detection and restoration models are moved to MPS.

### GUI on macOS/MPS (One‑Click Device Detection/Switch)

- Auto selection: On first run the app picks the best device by priority: `CUDA → MPS → CPU`. If you configured `cpu` but a faster device is available, it will upgrade automatically.
- Manual switch: Open the left Preferences sidebar → Mosaic Removal → `Device` dropdown.
  - The list shows available `CUDA` GPU names, `MPS` (when Apple Metal is available), and `CPU`.
  - Click your target device to apply immediately; detection/restoration models migrate in the background, and preview buffering adapts to keep playback smooth.
- CLI equivalents: `--device` accepts `cpu`, `cuda:X`, `mps`, for example:
  ```shell
  lada-cli --device cuda:0 --input <path>
  lada-cli --device mps --input <path>
  ```

> [!TIP]
> If playback stutters, increase the `Buffer duration` or reduce the `Maximum clip duration`. On MPS/CPU, tune batch size and resolution to balance latency and smoothness.

### Recommended defaults

- Mosaic Restoration: `basicvsrpp-v1.2` (Default)
- Mosaic Detection: `v3.1-fast` (Default; try `v3.1-accurate` or `v2` for different trade-offs)
- Max clip length: `180`
- Export codec: GUI `libx264` (CLI `h264` maps to `libx264`)
- CRF: `20` (quality vs size; NVENC uses `qp` instead)
- Device: auto-priority `CUDA → MPS → CPU` (overridable via GUI or CLI)
