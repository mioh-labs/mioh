...
class MosaicDetector:
    def __init__(self, model: Yolo11SegmentationModel, video_metadata: VideoMetadata, frame_detection_queue: PipelineQueue, mosaic_clip_queue: PipelineQueue, error_handler: Callable[[ErrorMarker], None], max_clip_length=30, clip_size=256, device: torch.device | None = None, pad_mode='reflect', batch_size=4):
        ...
        self._batch_adjust_check_interval = 12
        self._batch_adjust_counter = 0
    ...
    def _maybe_adjust_batch_size(self):
        if not self._adaptive_batching_enabled:
            return
        self._batch_adjust_counter += 1
        if self._batch_adjust_counter % self._batch_adjust_check_interval != 0:
            return

        available_gb = get_mps_available_memory_gb()
        mps_stats = get_mps_memory_stats()
        pressure_ratio = mps_stats.get("pressure_ratio")
        feeder_fill = self.frame_feeder_queue.qsize() / max(1, self.frame_feeder_queue.maxsize)
        infer_fill = self.inference_queue.qsize() / max(1, self.inference_queue.maxsize)
        target_batch = self.batch_size

        if (pressure_ratio is not None and pressure_ratio >= 0.90) or available_gb < 3.5 or feeder_fill > 0.85 or infer_fill > 0.85:
            target_batch = max(self._min_batch_size, self.batch_size - 1)
        elif (pressure_ratio is None or pressure_ratio < 0.65) and available_gb > 8.0 and feeder_fill < 0.40 and infer_fill < 0.40:
            target_batch = min(self._max_batch_size, self.batch_size + 1)

        if target_batch != self.batch_size:
            self.batch_size = target_batch
            logger.info(
                "MPS detect batch adjusted to %s (avail=%.1fGB pressure=%s feeder=%.2f infer=%.2f)",
                self.batch_size, available_gb,
                f"{pressure_ratio:.2f}" if pressure_ratio is not None else "n/a",
                feeder_fill, infer_fill
            )
