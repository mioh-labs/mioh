...
# MPS/CUDA対応: PyTorch TensorをNumPy配列に安全に変換
def tensor_to_numpy(tensor):
    ...
    if isinstance(tensor, torch.Tensor):
        # MPSまたはCUDAの場合、CPUに移動してからnumpy化
        if tensor.device.type != 'cpu':
            return tensor.cpu().numpy()
        else:
            return tensor.numpy()
    # すでにnumpy配列の場合はそのまま返す
    elif isinstance(tensor, np.ndarray):
        return tensor
    else:
        # その他の型（PIL Imageなど）はそのまま返す
        return tensor
...
def tensor2img(tensor, rgb2bgr=True, out_type=np.uint8, min_max=(0, 1)):
    ...
    for _tensor in tensor:
        ...
        n_dim = _tensor.dim()
        if n_dim == 4:
            img_np = tensor_to_numpy(make_grid(_tensor, nrow=int(math.sqrt(_tensor.size(0))), normalize=False))
            img_np = img_np.transpose(1, 2, 0)
            if rgb2bgr:
                img_np = cv2.cvtColor(img_np, cv2.COLOR_RGB2BGR)
        elif n_dim == 3:
            img_np = tensor_to_numpy(_tensor)
            img_np = img_np.transpose(1, 2, 0)
