...
def setup_argparser() -> argparse.ArgumentParser:
    ...
    group_general.add_argument('--mps-memory-fraction', type=float, default=None, help=_("Set torch.mps per-process memory fraction. If omitted, LADA_MPS_MEMORY_FRACTION env var is used when present."))
    ...
    group_restoration.add_argument('--restore-blend-feather', type=float, default=1.0, help=_('Multiplier for restored-region feathering. 1 uses 6 px inward and 20 px outward at 1080p, scaled with output resolution. 0 disables it. (default: %(default)s)'))
...
def main():
    ...
    if device.type == "mps":
        ...
        if memory_fraction is None:
            env_fraction = os.environ.get("LADA_MPS_MEMORY_FRACTION")
            if env_fraction:
                try:
                    memory_fraction = float(env_fraction)
                except ValueError:
                    memory_fraction = None
        configure_mps_runtime(memory_fraction, verbose=False)
        mps_stats = get_mps_memory_stats()
        if os.environ.get("LADA_LOG_MPS_MEMORY") == "1" and mps_stats.get("available") and mps_stats.get("recommended_max_bytes") is not None:
            print(_("MPS recommended max memory: {memory} GiB").format(
                memory=f"{mps_stats['recommended_max_bytes'] / (1024 ** 3):.1f}"
            ))
