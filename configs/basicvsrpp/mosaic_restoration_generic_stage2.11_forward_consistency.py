"""Known-operator continuation from the deployed HF2500 + FC2 EMA.

Only complete square-average cells wholly inside the unfeathered ROI and the
native crop contribute to the new loss.  The half-LSB dead zone removes the
known uint8 quantization floor, so the clean GT remains a valid solution.

For a same-seed control run, override
``model.mosaic_forward_consistency_loss.loss_weight=0.0`` and ``work_dir`` on
the command line.  Stage 2.10 remains unchanged as historical provenance.
"""
...
experiment_name = 'mosaic_restoration_generic_stage2.11_forward_consistency'
work_dir = f'./experiments/basicvsrpp/{experiment_name}'

initialization_checkpoint = 'model_weights/hf2500-plus-fc2-500-ema.pth'
train_manifest = (
    '/Volumes/Project_HD/lada_finetune_aozora_hikari/'
    'fc2_best_hf_v1/manifests/train-old560-plus-fc2-v1.jsonl'
)
# Keep the established validation set unchanged for direct comparison with
# HF2500 + FC2 and the optional zero-weight control arm.
validation_manifest = (
    '/Volumes/Project_HD/lada_finetune_aozora_hikari/'
...
randomness = dict(seed=20260808, deterministic=True)
