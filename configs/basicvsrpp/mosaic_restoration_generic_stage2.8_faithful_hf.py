"""Faithful high-frequency tail fine-tune from the deployed iter9000 EMA.

This is intentionally a short, independently-gated experiment.  Alignment,
flow, feature extraction, and all four propagation branches are frozen.  Only
the reconstruction tail is updated, with GT-only losses and no GAN or
perceptual teacher.
"""
...
experiment_name = 'mosaic_restoration_generic_stage2.8_faithful_hf'
work_dir = f'./experiments/basicvsrpp/{experiment_name}'

initialization_checkpoint = (
    '/Volumes/Project_HD/lada_finetune_aozora_hikari/'
    'basicvsrpp_iter9000_faithful_hf/initialization/'
    'iter9000-ema-as-generator.pth'
)
train_manifest = (
    '/Volumes/Project_HD/lada_finetune_aozora_hikari/'
...
model = dict(
    ...
    discriminator=None,
    # A weak full-frame anchor preserves the already-correct clean context.
    pixel_loss=dict(
        type='CharbonnierLoss', loss_weight=0.10, reduction='mean'),
    # The primary fidelity term is normalized by ROI area, so small masks are
    # not drowned out by unchanged pixels elsewhere in the crop.
    roi_pixel_loss=dict(
